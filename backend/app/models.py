from sqlalchemy import Column, String, Integer, Boolean, Float, DateTime, ForeignKey, Text, Enum as SQLEnum, ARRAY
...
class User(Base):
    ...
    training_programs = relationship("TrainingProgram", back_populates="user", lazy="select", cascade="all, delete-orphan")
    body_metrics = relationship("BodyMetric", back_populates="user", lazy="select", cascade="all, delete-orphan")
    ...
    trainer_notes = relationship("TrainerNote", back_populates="trainer", lazy="select", foreign_keys="[TrainerNote.trainer_id]", cascade="all, delete-orphan")
    client_notes = relationship("TrainerNote", back_populates="client", lazy="select", foreign_keys="[TrainerNote.client_id]", cascade="all, delete-orphan")
